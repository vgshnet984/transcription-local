#!/usr/bin/env python3
"""
Enhanced Test script for Basic UI (port 8000)
...
def test_basic_ui_enhanced():
    """Test the basic UI functionality with enhanced logging."""
    base_url = "http://localhost:8000"
    sample_audio = "examples/sample_audio/tes1_first_35s.wav"
    
    log_message("=" * 60, "TEST")
    log_message("Testing Basic UI (port 8000) - Enhanced Version", "TEST")
    log_message("=" * 60, "TEST")
    
    # Check if server is running
    log_message("Checking Basic UI server status...", "CHECK")
    try:
        response = requests.get(f"{base_url}/health", timeout=5)
        if response.status_code != 200:
            log_message("Basic UI server not responding", "ERROR")
            return False
        log_message("Basic UI server is running and healthy", "SUCCESS")
    except requests.exceptions.RequestException:
        log_message("Basic UI server not accessible", "ERROR")
        return False
    ...
    log_message("=" * 60, "TEST")
    log_message("Basic UI test completed successfully!", "SUCCESS")
    log_message("=" * 60, "TEST")
    return True

if __name__ == "__main__":
    success = test_basic_ui_enhanced()
