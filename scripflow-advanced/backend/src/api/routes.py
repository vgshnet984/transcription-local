...
from src.database.models import AudioFile, TranscriptionJob, Transcription, TranscriptionSegment
from src.audio.processor import AudioProcessor
from src.transcription.engine import TranscriptionEngine, transcription_engine
    ...
    try:
        ...
        def run_transcription():
            try:
                ...
                # Update job progress to indicate model loaded
                background_job = background_db.query(TranscriptionJob).filter(TranscriptionJob.id == job.id).first()
                if background_job:
                    background_job.progress = 10.0  # Model loaded
                    background_job.status = "model_loaded"
                ...
                # Update progress to indicate transcription starting
                background_job = background_db.query(TranscriptionJob).filter(TranscriptionJob.id == job.id).first()
                if background_job:
                    background_job.progress = 20.0  # Transcription starting
                    background_job.status = "transcribing"
                    background_db.commit()
